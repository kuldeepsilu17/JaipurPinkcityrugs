...
export const AnnouncementBar: React.FC = () => {
  ...
  return (
    <aside aria-label="Announcement" className="bg-warmbrown-900 text-cream-100 text-xs py-2 px-4 transition-all duration-300 border-b border-warmbrown-800">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex-1 text-center flex items-center justify-center space-x-2 animate-fade-in key={currentIdx}">
          <Sparkles className="w-3 h-3 text-ochre-400 shrink-0" />
          <Link
            href={activeMsg.link}
            className="hover:text-ochre-300 transition-colors font-medium tracking-wide flex items-center space-x-1"
          >
            <span>{activeMsg.text}</span>
            <ChevronRight className="w-3 h-3 inline opacity-70" />
          </Link>
        </div>
